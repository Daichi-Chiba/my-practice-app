...
@php
  $lesson06Hints = [
      new \Illuminate\Support\HtmlString('<p>Sanctum のトークン発行テストは、DB の <code>personal_access_tokens</code> をアサートすることで漏れなく検証できます。</p>'),
      new \Illuminate\Support\HtmlString('<p>Policy でロール判定する際は <code>$user->hasRole()</code> のようなヘルパを作ると読みやすくなります。</p>'),
      new \Illuminate\Support\HtmlString('<p>RateLimiter の設定を変更したら <code>php artisan config:clear</code> を忘れずに。テストでは <code>for</code> だけでなく <code>limiter</code> のキーもチェックしましょう。</p>')
  ];

  $lesson06Answer = new \Illuminate\Support\HtmlString(view('exercises.partials.lesson06.answer')->render());
@endphp

<x-exercise.reveal id="lesson06-overview" :hints="$lesson06Hints" :answer="$lesson06Answer" />

<footer class="exercise-footer">
  <a class="exercise-footer__link" href="{{ route('lesson05') }}">
