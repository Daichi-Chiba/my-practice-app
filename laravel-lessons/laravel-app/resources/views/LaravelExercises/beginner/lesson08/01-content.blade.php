...
@php
  $lesson08Hints = [
      new \Illuminate\Support\HtmlString('<p>v1 と v2 のコードをフォルダで分ける際は、FormRequest や Resource もバージョン別に配置すると読みやすくなります。</p>'),
      new \Illuminate\Support\HtmlString('<p>429 応答の UX を確認するために <code>php artisan tinker</code> でループして叩くなど再現手段を用意しましょう。</p>'),
      new \Illuminate\Support\HtmlString('<p>OpenAPI の CI は <code>spectral</code> や <code>openapi-diff</code> を使うと差分検出が自動化できます。</p>'),
  ];

  $lesson08Answer = new \Illuminate\Support\HtmlString('<p><strong>解答例:</strong> <code>routes/api.php</code> に <code>Route::prefix("v1")</code> と <code>Route::prefix("v2")</code> を追加し、Resource をバージョン別フォルダに配置。RateLimiter で v1=60req/min, v2=120req/min とし、429 では <code>{"message":"Too Many Requests","retry_after":5}</code> を返却。OpenAPI ファイルは <code>docs/openapi.yml</code> にまとめ、GitHub Actions で <code>redoc-cli bundle</code> と <code>speccy lint</code> を実行して PR でプレビューを生成しました。</p>');
@endphp

<x-exercise.reveal id="lesson08-overview" :hints="$lesson08Hints" :answer="$lesson08Answer" />
