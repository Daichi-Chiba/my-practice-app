...
<header class="exercise-hero">
  ...
  <p class="exercise-hero__lead">
    占いアプリのドメインを整理し、マイグレーション・シーディング・リレーションを揃えます。
    Eloquent で扱いやすいモデル構造を意識しながら、API と管理画面双方で再利用できるデータ設計を完成させましょう。
  </p>
...
<section class="exercise-card">
  ...
  <div class="exercise-card__content">
    <p class="exercise-card__description">
      開発環境で使い回せるダミーデータを整えます。モデルファクトリを活用し、高速にテストデータを生成しましょう。
    </p>
    <ol class="exercise-card__steps">
...
<section class="exercise-card">
  ...
  <div class="exercise-card__content">
    <p class="exercise-card__description">
      モデル間の関連を定義し、読み書き双方で正しく機能するかを確認します。命名と戻り値の型ヒントも整理しましょう。
    </p>
    <ol class="exercise-card__steps">
    ...
    <div class="exercise-warning">
      <strong>注意:</strong> リレーション名は複数形・単数形の規約に従い、テストで <code>$user->fortunes()->create()</code> が動くかチェックしてください。
    </div>
  </div>
</section>
...
<section class="exercise-checklist">
  ...
  <ul class="exercise-checklist__list">
    <li>✅ ER 図が README または docs/ に添付され、主要テーブルとリレーションが説明されている</li>
...
@php
  $lesson03Hints = [
      new \Illuminate\Support\HtmlString('<p>ER 図はツールで描くだけでなく、各エンティティの責務をコメントで書き添えるとレビューがスムーズです。</p>'),
      new \Illuminate\Support\HtmlString('<p>マイグレーションでは <code>foreignId()->constrained()</code> を活用すると外部キーとインデックスがまとめて定義できます。</p>'),
      new \Illuminate\Support\HtmlString('<p>N+1 の検出には <code>->with()</code> の指定漏れだけでなく、Collection 側でループしていないかも確認しましょう。</p>'),
  ];

  $lesson03Answer = new \Illuminate\Support\HtmlString(view('LaravelExercises.beginner.lesson03.answer')->render());
@endphp

<x-exercise.reveal id="lesson03-overview" :hints="$lesson03Hints" :answer="$lesson03Answer" />
