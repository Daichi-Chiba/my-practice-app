...
<section class="exercise-card">
  ...
  <div class="exercise-card__content">
    ...
    <ol class="exercise-card__steps">
      ...
      <li>Feature テストで不正値送信時に 422 とバリデーションメッセージが返ることを検証</li>
    </ol>
  </div>
</section>
...
<section class="exercise-card">
  ...
  <div class="exercise-card__content">
    <p class="exercise-card__description">
      API と Web で統一されたエラーレスポンスを返せるよう、<code>app/Exceptions/Handler.php</code> を拡張します。
    </p>
    <ol class="exercise-card__steps">
    ...
    <div class="exercise-warning">
      <strong>注意:</strong> ログへはスタックトレースを含めますが、レスポンスには機密情報を載せないようにしましょう。
    </div>
  </div>
</section>
...
@php
  $lesson05Hints = [
      new \Illuminate\Support\HtmlString('<p>FormRequest には <code>authorize()</code> も忘れずに。ここで <code>true</code> を返し忘れると 403 になってしまいます。</p>'),
      new \Illuminate\Support\HtmlString('<p><code>Handler::renderable()</code> では <code>request()->expectsJson()</code> を使うと Web/ API 双方に対応しやすいです。</p>'),
      new \Illuminate\Support\HtmlString('<p>Slack 通知はローカルでは送らず、スタブを用意するとテストが安定します。</p>'),
  ];

  $lesson05Answer = new \Illuminate\Support\HtmlString(view('exercises.partials.lesson05.answer')->render());
@endphp

<x-exercise.reveal id="lesson05-overview" :hints="$lesson05Hints" :answer="$lesson05Answer" />
