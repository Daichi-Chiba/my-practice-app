...
<section class="exercise-card">
  ...
  <div class="exercise-card__content">
    ...
    <ol class="exercise-card__steps">
      <li><code>config/horizon.php</code> を編集し、default / critical キューを使い分ける構成を作成</li>
...
<section class="exercise-card">
  ...
  <div class="exercise-card__content">
    ...
    <ol class="exercise-card__steps">
      <li><code>$tries</code> / <code>$backoff</code> / <code>retryUntil()</code> を設定し、恒久的エラー時に <code>fail()</code> を呼び出す</li>
...
@php
  $lesson09Hints = [
      new \Illuminate\Support\HtmlString('<p>ジョブの冪等性を担保するには、履歴テーブルにユニーク制約を設定し <code>firstOrCreate</code> で登録するのがおすすめです。</p>'),
      new \Illuminate\Support\HtmlString('<p>Horizon の構成は <code>php artisan horizon:publish</code> で出力される設定ファイルを用いてカスタマイズするのが早道です。</p>'),
      new \Illuminate\Support\HtmlString('<p>失敗ジョブの通知は <code>Queue::failing</code> イベントで Slack 通知を送ると実装がシンプルになります。</p>'),
  ];

  $lesson09Answer = new \Illuminate\Support\HtmlString('<p><strong>解答例:</strong> 結果通知メールを <code>NotifyFortuneResultJob</code> に移行し、メール送信前に <code>notifications</code> テーブルをチェックして二重送信を防止。Horizon 設定では default と critical の二系統を定義し、critical には 3 worker / 1 processes を割り当て。失敗ジョブは <code>Queue::failing</code> で Slack 通知し、AggregateFortuneResultJob を日次スケジュールに登録、成功時は Slack へ報告するようにしました。</p>');
@endphp

<x-exercise.reveal id="lesson09-overview" :hints="$lesson09Hints" :answer="$lesson09Answer" />
